...
internal sealed class EditorSession
{
    ...
    public string GetVisibleLine(int line)
    {
        string text = ExpandTabs(Document.LineAt(line).ToString());
        if (ViewLeft >= text.Length)
        {
            return string.Empty;
        }

        int span = Math.Min(Layout.Columns - LineNumberMargin, text.Length - ViewLeft);
        return text.Substring(ViewLeft, Math.Max(0, span));
    }
